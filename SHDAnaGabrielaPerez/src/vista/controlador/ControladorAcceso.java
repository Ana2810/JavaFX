...
public class ControladorAcceso implements Initializable  {
	...
	public class Click implements EventHandler<MouseEvent>{
	 ...
	 public class validarLetras implements EventHandler<KeyEvent> {
		...
		public void handle(KeyEvent e) {
			...
			if (txtnombre.getText().length()<10){
				char letra;
			    letra=(e.getCharacter()+"").toUpperCase().charAt(0);
			    if ((letra+"").matches("[A-Z Á-Ú]")){
	 ...
	 public class validarNumeros implements EventHandler<KeyEvent>{
	...
	@FXML protected void eliminar(ActionEvent e){
		...
		if(carreraModificada!=null){
			int opcion=JOptionPane.showOptionDialog(null, "¿Seguro que quiere eliminar los datos?", "ADVERTENCIA", JOptionPane.YES_NO_OPTION, MessageType.WARNING.ordinal(), null, null, null);
			if(opcion==0){
	...
	@FXML protected void modificar(ActionEvent e){
		...
		if(carreraModificada!=null){
			int opcion=JOptionPane.showOptionDialog(null, "¿Seguro que quiere modificar los datos?", "ADVERTENCIA", JOptionPane.YES_NO_OPTION, MessageType.WARNING.ordinal(), null, null, null);
			if(opcion==0){
	...
	@FXML protected void salir(){
		int opcion=JOptionPane.showOptionDialog(null, "¿Seguro que quiere salir?", "ADVERTENCIA",JOptionPane.YES_NO_OPTION ,MessageType.WARNING.ordinal() , null, null, null);
		if(opcion==0){
				System.exit(0);
	}
	}	
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		nombre.setPrefWidth(75.0);
		siglas.setPrefWidth(75.0);
		jefe.setPrefWidth(75.0);
		Matricula.setPrefWidth(75.0);
		acreditada.setPrefWidth(75.0);

		
		LlenarTabla();
		jtable.setOnMouseClicked(new Click());
		txtJefeCarrera.addEventFilter(KeyEvent.KEY_TYPED,new validarLetras());
		txtnombre.addEventFilter(KeyEvent.KEY_TYPED,new validarLetras());
		txtMatricula.addEventFilter(KeyEvent.KEY_TYPED,new validarNumeros());
